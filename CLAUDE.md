...
- Keep `PricingTable` provider-invariant with one rate per priced category. A provider billing one category at several rates gets a concrete table in its backend subpackage that spends them all and reports their sum, so what crosses into `Usage` is the priced categories and never a rate.
...
- Verified provider facts that cross module boundaries (checked against anthropic 0.120.0 / openai 2.45.0): both SDK clients retry internally by default (`DEFAULT_MAX_RETRIES = 2`) and honor `retry-after-ms`/`retry-after` only up to 60 seconds; both SDKs' `APIStatusError` exposes `.response` (an httpx.Response) whose headers carry retry-after; `with_options(max_retries=0)` returns a client copy on all six client classes the adapters accept; both SDKs' `_base_client._should_retry` is byte-identical, honoring `x-should-retry` when the header says `true` or `false` and otherwise retrying 408, 409, 429, and every status at or above 500. Each provider's request and response `service_tier` vocabularies are separate facts: anthropic's are disjoint words, and openai's share one literal whose response value the API documents as possibly differing from the requested one, so the tier that prices a response is read off that response and never assumed from the request.
...
- `pricing.py`: the neutral rate table and the arithmetic that spends it; imports no SDK and no error class.
