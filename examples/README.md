...
| [`09_batch_failures.py`](09_batch_failures.py) | a batch whose middle item the adapter will not send: every slot settles, `to_row` renders all three, `Usage.sum_of` totals the spend over successes and failures, and the failed slots map back to the conversations to resubmit |
