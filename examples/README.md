...
| [`04_failures_and_deadlines.py`](04_failures_and_deadlines.py) | `run_batch_and_handle_what_failed`: one `SharedBackoff` per account, a batch item the adapter will not send, `max_working_seconds_per_item`, and each failed item failed over to a second provider under `timeout_seconds` |
